        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = result {
                    error!(
                        "Heartbeat failed: {:?}",
                        FerrumDepositError::HeartBeatFailed(e.to_string())
                    );
                }
            }
        });
...
#[tonic::async_trait]
impl DepositDataNodeService for DataNode {
    ...
    async fn get_block_streamed(
        ...
        let block_id_uuid = Uuid::parse_str(&block_id).map_err(|_| {
            Status::from(FerrumDepositError::GetBlockStreamedError(
        ...
        let block_info = blocks_guard.get(&block_id_uuid).ok_or(Status::from(
            FerrumDepositError::GetBlockStreamedError("Block not found.".to_string()),
        ))?;
        ...
        let mut file = File::open(&file_path).await.map_err(|_| {
            Status::from(FerrumDepositError::GetBlockStreamedError(format!(
    ...
    async fn start_block_stream(
        ...
        let block_uuid = Uuid::parse_str(&block_stream_info.block_id)
            .map_err(|_| FerrumDepositError::UUIDError(String::from("Bad UUID string.")))?;
